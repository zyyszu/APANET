function sessionID= trainWeakly1(dbTrain, dbVal, varargin)
    ...
    if opts.startEpoch<2
        ...
        if isempty(opts.dbCheckpoint0)
            opts.dbCheckpoint0= sprintf('%s%s_%s_%s_%s_%sdb.bin', opts.outPrefix, dbTrain.name, opts.sessionID, opts.layerName, opts.method, opts.checkpoint0suffix);
        end
        if isempty(opts.qCheckpoint0)
            opts.qCheckpoint0= sprintf('%s%s_%s_%s_%s_%sq.bin', opts.outPrefix, dbTrain.name, opts.sessionID, opts.layerName, opts.method, opts.checkpoint0suffix);
        end
        if isempty(opts.dbCheckpoint0val)
            opts.dbCheckpoint0val= sprintf('%s%s_%s_%s_%s_%sdb.bin', opts.outPrefix, dbVal.name, opts.sessionID, opts.layerName, opts.method, opts.checkpoint0suffix);
        end
        if isempty(opts.qCheckpoint0val)
            opts.qCheckpoint0val= sprintf('%s%s_%s_%s_%s_%sq.bin', opts.outPrefix, dbVal.name, opts.sessionID, opts.layerName, opts.method, opts.checkpoint0suffix);
        end
    ...
    for iEpoch= 1:opts.nEpoch
        ...
        %exponential decay over iepoch
        oldLr= opts.learningRate;
        freq = exp(-(iEpoch-1)* 0.1);
        lr = oldLr* freq;

        oldbatchCompFeatsFrequency= batchCompFeatsFrequency;
        nowbatchCompFeatsFrequency= round(oldbatchCompFeatsFrequency/freq); 

        if nowbatchCompFeatsFrequency>1000
            nowbatchCompFeatsFrequency = 1000;
        end
        ...
        for iBatch= 1:nBatches
            ...
            if rem(iBatch, nowbatchCompFeatsFrequency)==0 && iBatch~=1 && iBatch~=nBatches
                clear qFeat dbFeat;
